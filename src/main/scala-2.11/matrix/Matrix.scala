...
final case class Matrix(rows: Vector[Vector[Int]]) {
  def addCol(col: Seq[Int]): Matrix = {
    require((rows.nonEmpty && rows.size == col.size) || rows.isEmpty, "Dimension of the new column is wrong.")
  ...
  def addRow(row: Seq[Int]): Matrix = {
    require((rows.nonEmpty && rows.head.size == row.size) || rows.isEmpty, "Dimension of the new row is wrong.")
  ...
  def * (right: Matrix) : Matrix = {
    ...
    require(rows.nonEmpty && right.rows.nonEmpty, "Matrices cannot be empty")
